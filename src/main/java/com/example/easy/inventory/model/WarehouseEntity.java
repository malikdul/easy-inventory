...
@SqlResultSetMappings({ 
	@SqlResultSetMapping(name = "getWarehouseFrequencyReport", classes = {
	...
	@SqlResultSetMapping(name = "getWarehouseFrequencyById", columns = {
		@ColumnResult(name = "count", type = Integer.class)
	})
...
@NamedNativeQueries({
	...
	@NamedNativeQuery(name = "getWarehouseFrequencyReport", query = "SELECT s.name name, COUNT(s.name) count "
			+ "FROM warehouse s "
			+ "LEFT JOIN orders o on o.warehouseId = s.id "
			+ "WHERE s.accountId = :accountId "
			+ "GROUP BY s.name ", resultSetMapping = "getWarehouseFrequencyReport"),
	
	
	@NamedNativeQuery(name = "getWarehouseFrequencyById", query = "SELECT COUNT(s.name) count "
			+ "FROM warehouse s "
			+ "LEFT JOIN orders o on o.warehouseId = s.id "
			+ "WHERE s.accountId = :accountId AND s.id = :warehouseId "
			+ "GROUP BY s.name ", resultSetMapping = "getWarehouseFrequencyById"),
...
public class WarehouseEntity implements Serializable {
    ...
    public WarehouseEntity name(String name) {
        this.name = name;
        return this;
    }
    ...
    public WarehouseEntity type(String type) {
        this.type = type;
        return this;
    }
