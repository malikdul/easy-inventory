...
public class StockServiceImpl implements IStockService {
	...
	public Collection<StockDTO> getAll(Integer accountId, Pageable pageable) {
		...
		return IStockMapper.INSTANCE.stocksPagedToStockDTOs(stockRepository.findAllByAccountId(accountId, pageable))
				.stream().map(stock -> {
					stock.setProduct(productService.findOne(stock.getProductId()));
					if (null != stock.getWarehouseId()) {
						stock.setWarehouse(warehouseService.findOne(stock.getWarehouseId()));
					}
					return stock;
